/* @flow strict-local */
import getAutocompleteFilter from '../getAutocompleteFilter';

describe('getAutocompleteFilter', () => {
  let selection = { start: 0, end: 0 };
  test('get empty object for empty text', () => {
    expect(getAutocompleteFilter('', selection)).toEqual({ filter: '', autocompleteType: '' });
  });

  test('get @users filters', () => {
    selection = { start: 1, end: 1 };
    expect(getAutocompleteFilter('@', selection)).toEqual({ filter: '', autocompleteType: '@' });

    selection = { start: 3, end: 3 };
    expect(getAutocompleteFilter('@ab', selection)).toEqual({
      filter: 'ab',
      autocompleteType: '@',
    });

    selection = { start: 6, end: 6 };
    expect(getAutocompleteFilter('@ab cd', selection)).toEqual({
      filter: 'ab cd',
      autocompleteType: '@',
    });

    selection = { start: 5, end: 5 };
    expect(getAutocompleteFilter('abc @ ', selection)).toEqual({
      filter: '',
      autocompleteType: '@',
    });
  });

  test('get respective autocompleteType even when keyword is entered in new line', () => {
    selection = { start: 1, end: 1 };
    expect(getAutocompleteFilter('\n@', selection)).toEqual({ filter: '', autocompleteType: '' });
    expect(getAutocompleteFilter('\n#', selection)).toEqual({ filter: '', autocompleteType: '' });
    expect(getAutocompleteFilter('\n:', selection)).toEqual({ filter: '', autocompleteType: '' });

    selection = { start: 2, end: 2 };
    expect(getAutocompleteFilter('\n@', selection)).toEqual({ filter: '', autocompleteType: '@' });
    expect(getAutocompleteFilter('\n#', selection)).toEqual({ filter: '', autocompleteType: '#' });
    expect(getAutocompleteFilter('\n:', selection)).toEqual({ filter: '', autocompleteType: ':' });
  });

  test('get #streams filters', () => {
    selection = { start: 1, end: 1 };
    expect(getAutocompleteFilter('#', selection)).toEqual({ filter: '', autocompleteType: '#' });

    selection = { start: 3, end: 3 };
    expect(getAutocompleteFilter('#ab', selection)).toEqual({
      filter: 'ab',
      autocompleteType: '#',
    });

    selection = { start: 6, end: 6 };
    expect(getAutocompleteFilter('#ab cd', selection)).toEqual({
      filter: 'ab cd',
      autocompleteType: '#',
    });

    selection = { start: 5, end: 5 };
    expect(getAutocompleteFilter('abc # ', selection)).toEqual({
      filter: '',
      autocompleteType: '#',
    });
  });

  test('get :emoji filters', () => {
    selection = { start: 1, end: 1 };
    expect(getAutocompleteFilter(':', selection)).toEqual({ filter: '', autocompleteType: ':' });

    selection = { start: 3, end: 3 };
    expect(getAutocompleteFilter(':ab', selection)).toEqual({
      filter: 'ab',
      autocompleteType: ':',
    });

    selection = { start: 6, end: 6 };
    expect(getAutocompleteFilter(':ab cd', selection)).toEqual({
      filter: 'ab cd',
      autocompleteType: ':',
    });

    selection = { start: 5, end: 5 };
    expect(getAutocompleteFilter('abc : ', selection)).toEqual({
      filter: '',
      autocompleteType: ':',
    });
  });

  test('get filter for more complicated text', () => {
    selection = { start: 9, end: 9 };
    expect(getAutocompleteFilter(':smile@ab', selection)).toEqual({
      filter: 'smile@ab',
      autocompleteType: ':',
    });

    selection = { start: 2, end: 2 };
    expect(getAutocompleteFilter(':s@ab', selection)).toEqual({
      filter: 's',
      autocompleteType: ':',
    });

    selection = { start: 12, end: 12 };
    expect(getAutocompleteFilter(':smile@ab cd', selection)).toEqual({
      filter: 'smile@ab cd',
      autocompleteType: ':',
    });

    selection = { start: 3, end: 3 };
    expect(getAutocompleteFilter('@ab :cd #a b', selection)).toEqual({
      filter: 'ab',
      autocompleteType: '@',
    });

    selection = { start: 7, end: 7 };
    expect(getAutocompleteFilter('@ab :cd #a b', selection)).toEqual({
      filter: 'cd',
      autocompleteType: ':',
    });

    selection = { start: 12, end: 12 };
    expect(getAutocompleteFilter('@ab :cd #a b', selection)).toEqual({
      filter: 'a b',
      autocompleteType: '#',
    });

    selection = { start: 8, end: 8 };
    expect(getAutocompleteFilter(':#@smile', selection)).toEqual({
      filter: 'smile',
      autocompleteType: '@',
    });

    selection = { start: 1, end: 1 };
    expect(getAutocompleteFilter(':#@smile', selection)).toEqual({
      filter: '',
      autocompleteType: ':',
    });

    selection = { start: 11, end: 11 };
    expect(getAutocompleteFilter('@r@::@q@m p', selection)).toEqual({
      filter: '@q@m p',
      autocompleteType: ':',
    });

    selection = { start: 1, end: 1 };
    expect(getAutocompleteFilter('@r@::@q@m p', selection)).toEqual({
      filter: '',
      autocompleteType: '@',
    });

    selection = { start: 16, end: 16 };
    expect(getAutocompleteFilter('some @ab:cd #a b', selection)).toEqual({
      filter: 'a b',
      autocompleteType: '#',
    });
  });
});
